...
class Smartsize(ImageProcessor):
    width = None
    height = None
    max_width = None
    max_height = None
    ...
    def process(cls, img, fmt, obj):
        cur_width, cur_height = img.size
        if cls.crop:
            crop_horz = cls.crop_horz_field
            crop_vert = cls.crop_vert_field
            ratio = max(float(cls.width)/cur_width, float(cls.height)/cur_height)
            resize_x, resize_y = ((cur_width * ratio), (cur_height * ratio))
            if max_width: resize_x = min(resize_x, max_width)
            if max_height: resize_y = min(resize_y, max_height)
            crop_x, crop_y = (abs(cls.width - resize_x), abs(cls.height - resize_y))
            x_diff, y_diff = (int(crop_x / 2), int(crop_y / 2))
            box_left, box_right = {
                0: (0, cls.width),
            ...
            new_dimensions = (int(round(cur_width*ratio)),
                              int(round(cur_height*ratio)))
            # never let the resized image be greater than max_width/max_height
            if max_width: new_dimensions[0] = min(new_dimensions[0], max_width)
            if max_height: new_dimensions[1] = min(new_dimensions[1], max_height)
            if new_dimensions[0] > cur_width or \
               new_dimensions[1] > cur_height:
                if not cls.upscale:
                    return img, fmt
