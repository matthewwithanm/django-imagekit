...
class ThumbnailAssignmentNode(template.Node):
    ...
    def render(self, context):
        ...
        if kwargs.get('anchor'):
            # ImageKit uses pickle at protocol 0, which throws infinite recursion errors
            # when anchor is set to a SafeString instance. This converts the SafeString
            # into a str instance.
            kwargs['anchor'] = repr(kwargs['anchor'])[1:-1]
        generator = generator_registry.get(generator_id, **kwargs)
